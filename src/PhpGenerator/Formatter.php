...
class Formatter
{
	...
	private static function doDump(&$var, int $level = 0)
	{
		if ($var instanceof PhpLiteral) {
			...
			if (empty($var)) {
				...
				foreach ($var as $k => &$v) {
					if ($k !== $marker) {
						$item = ($k === $counter ? '' : self::doDump($k, $level + 1) . ' => ') . self::doDump($v, $level + 1);
			...
			$var = serialize($var);
			return 'unserialize(' . self::doDump($var, $level) . ')';
			...
			if ($level > self::MAX_DEPTH || in_array($var, $list, true)) {
				...
				foreach ($arr as $k => &$v) {
					if (!isset($props) || isset($props[$k])) {
						$out .= "$space\t" . self::doDump($k, $level + 1) . ' => ' . self::doDump($v, $level + 1) . ",\n";
					}
				}
				array_pop($list);
				$out .= $space;
			...
			return $class === 'stdClass'
				? "(object) [$out]"
				: Helpers::class . "::createObject('$class', [$out])";
